...
Native Rust companion to the web interface at `cc.minecartchris.cc`. Same WebSocket protocol, same tokens. **Focus is file editing and terminal viewing** with keyboard input support.
...
### URL
```
wss://cc.minecartchris.cc/connect?id=<token>&capabilities=file:edit,terminal:view
```
For file editing only, capability set is `file:edit`. For terminal viewing, append `terminal:view`.
...
- [ ] **Terminal keyboard input.** Map egui `Key` → CC key names (port `keyName()` from cc-web-term). Send `Packet::TerminalEvents` on keydown/keyup.
- [ ] **Lua syntax highlighting.** Drop in `syntect` with the Lua `.sublime-syntax`.
