...
pub struct TerminalContents {
    ...
    pub cur_fore: Option<String>,
    pub cur_back: Option<String>,
...
fn encode_to_value(packet: &Packet) -> Result<Value> {
    ...
    match packet {
        ...
        Packet::TerminalContents(contents) => {
            obj.insert("width".into(), Value::from(contents.width));
            obj.insert("height".into(), Value::from(contents.height));
            obj.insert("cursorX".into(), Value::from(contents.cursor_x));
            obj.insert("cursorY".into(), Value::from(contents.cursor_y));
            obj.insert("cursorBlink".into(), Value::from(contents.cursor_blink));
            if let Some(ref cf) = contents.cur_fore { obj.insert("curFore".into(), Value::from(cf.clone())); }
            if let Some(ref cb) = contents.cur_back { obj.insert("curBack".into(), Value::from(cb.clone())); }
            if let Some(ref pal) = contents.palette { obj.insert("palette".into(), serde_json::to_value(pal).unwrap_or_default()); }
            obj.insert("text".into(), serde_json::to_value(&contents.text).unwrap_or_default());
            obj.insert("fore".into(), serde_json::to_value(&contents.fore).unwrap_or_default());
            obj.insert("back".into(), serde_json::to_value(&contents.back).unwrap_or_default());
        }
...
pub fn decode(s: &str) -> Result<Packet> {
    ...
    Ok(match code {
        ...
        PacketCode::TerminalContents => Packet::TerminalContents({
            let width = obj.get("width").and_then(|v| v.as_u64()).unwrap_or(80) as u32;
            let height = obj.get("height").and_then(|v| v.as_u64()).unwrap_or(25) as u32;
            let cursor_x = obj.get("cursorX").and_then(|v| v.as_u64()).unwrap_or(1) as u32;
            let cursor_y = obj.get("cursorY").and_then(|v| v.as_u64()).unwrap_or(1) as u32;
            let cursor_blink = obj.get("cursorBlink").and_then(|v| v.as_bool()).unwrap_or(true);
            let cur_fore = obj.get("curFore").and_then(|v| v.as_str()).map(String::from);
            let cur_back = obj.get("curBack").and_then(|v| v.as_str()).map(String::from);
            let palette = obj.get("palette").and_then(|v| {
                serde_json::from_value(v.clone()).ok()
            });
            let text = obj.get("text").and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            let fore = obj.get("fore").and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            let back = obj.get("back").and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            TerminalContents { width, height, cursor_x, cursor_y, cursor_blink, cur_fore, cur_back, palette, text, fore, back }
        }),
