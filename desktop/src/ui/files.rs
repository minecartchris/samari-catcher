        ...
        .show_inside(ui, |ui| {
            ...
            ScrollArea::vertical().show(ui, |ui| {
                ...
                for (name, modified, read_only, is_new) in rows {
                    ...
                    ui.horizontal(|ui| {
                        let label = if modified { format!("● {name}") } else { name.clone() };
...
fn welcome_screen(session: &Session, ui: &mut Ui) {
    ui.vertical_centered(|ui| {
        ui.add_space(40.0);
        ui.heading("Samurai Catcher — desktop");
        ui.label(
            "Use `samari edit <file>` on the computer to open a file here, \
        ...
        ui.group(|ui| {
            ...
            ui.code(format!(
                "wget https://cc.minecartchris.cc/samari.lua\nsamari.lua {}",
