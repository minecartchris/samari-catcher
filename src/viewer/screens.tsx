...
const genSetup = (token: string) =>
  <div>
    <pre>
      <span class={termLine}>wget {window.location.origin}/samari.lua</span>
      {"\n"}
      <span class={termLine}>samari.lua {token}</span>
    </pre>
  </div>;
...
export const TokenDisplay = ({ token }: TokenDisplayProps) =>
  <div class={infoContainer}>
    ...
    <div class={infoDescription}>
      ...
      <p>
        The <code>samari.lua</code> program can be run with <code>--help</code> to
        see a full list of options, but here's a look at some of the highlights:
      </p>
      <ul>
        <li><code>samari -tnone TOKEN</code> Don't show the terminal remotely</li>
        <li>
          <code>samari -t80x30 TOKEN</code> Use a terminal 80 characters wide and
          30 high. Note this will hide the terminal on the local computer.
        </li>
        <li>Use <code>samari edit FILE</code> to edit a file within your browser</li>
        <li>
